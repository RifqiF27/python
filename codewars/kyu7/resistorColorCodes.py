...
def decode_resistor_colors(bands):
    ...
    resistor = 10 * colors[bands[0]] + colors[bands[1]]
    resistor *= 10 ** colors[bands[2]]
    # print(resistor)

    tolerance = "20%"

    if len(bands) > 3:
        tolerance = "5%" if bands[3] == "gold" else "10%"

    if resistor < 1000:
        resistor_str = str(resistor)
    elif resistor < 1000000:
        resistor_str = f"{
            resistor / 1000:.1f}k" if (resistor % 1000) != 0 else f"{int(resistor / 1000)}k"
    else:
        resistor_str = f"{
            resistor / 1000000:.1f}M" if (resistor % 1000000) != 0 else f"{int(resistor / 1000000)}M"

    return f"{resistor_str} ohms, {tolerance}"


print(decode_resistor_colors("brown black black"), "10 ohms, 20%")
print(decode_resistor_colors("brown black brown gold"), "100 ohms, 5%")
print(decode_resistor_colors("red red brown"), "220 ohms, 20%")
print(decode_resistor_colors("orange orange brown gold"), "330 ohms, 5%")
print(decode_resistor_colors("yellow violet brown silver"), "470 ohms, 10%")
print(decode_resistor_colors("blue gray brown"), "680 ohms, 20%")
print(decode_resistor_colors("brown black red silver"), "1k ohms, 10%")
print(decode_resistor_colors("brown black orange"), "10k ohms, 20%")
print(decode_resistor_colors("red red orange silver"), "22k ohms, 10%")
print(decode_resistor_colors("yellow violet orange gold"), "47k ohms, 5%")
print(decode_resistor_colors("brown black yellow gold"), "100k ohms, 5%")
print(decode_resistor_colors("orange orange yellow gold"), "330k ohms, 5%")
print(decode_resistor_colors("red black green gold"), "2M ohms, 5%")
